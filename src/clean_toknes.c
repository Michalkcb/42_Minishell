...
/*
Funkcja `ft_remove_token` usuwa wskazany token `*to_del` z listy tokenów, której głową jest `*head`. Jeśli usuwany token jest pierwszym w liście, aktualizuje wskaźnik głowy na następny element, zwalnia pamięć tekstu i struktury tokenu, a następnie ustawia `*to_del` na nową głowę. W przeciwnym razie, aktualizuje wskaźnik `next` poprzedniego tokenu (`*prev`) tak, by omijał usuwany, zwalnia pamięć usuwanego tokenu, i przestawia `*to_del` na kolejny element listy. Funkcja zapewnia poprawne zarządzanie pamięcią i strukturą listy tokenów.
*/
